...
export class ProfileEntryEditComponent implements OnInit {
  ...
  private createAchievementsForm(): void {
    this.entryForm = this.fb.group({
      ...
      deliveryMode: [_.get(this.entryDetails, 'deliveryMode', '')],
      startDate: [_.get(this.entryDetails, 'startDate', ''), [startDateValidator('endDate')]],
      endDate: [_.get(this.entryDetails, 'endDate', ''), [endDateValidator('startDate')]],
      issuedDate: [_.get(this.entryDetails, 'issuedDate', ''), [Validators.required]],
      learningHours: [_.get(this.entryDetails, 'learningHours', ''), [Validators.pattern(/^\d+$/), Validators.min(1), Validators.max(1000)]],
      trainingType: [_.get(this.entryDetails, 'trainingType', ''), [Validators.required]],

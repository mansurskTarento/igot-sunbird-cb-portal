...
// ...existing
  ...
  getDetailsFromRoutes() {
      this.activatedRoute.data.subscribe(data => {
        if (_.get(data, 'peopleSuggestions.data', []).length > 0) {
          this.recommendationList = _.get(data, 'peopleSuggestions.data', [])
        }
      })
    }
  ...
  getRecommendationsList() {
    ...
    this.networkingSvc.getRecommendedUsers(formBody).subscribe({
      next: (response) => {
        this.recommendationList = _.get(response, 'result.data.results', []);
